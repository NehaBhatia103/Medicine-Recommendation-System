...
# Function to get all the details of the disease
...
def get_disease_name(patient_symptoms):
...
def predict():
    if request.method == 'POST':
        ...
        # removing any extra spaces or characters , if any
        user_symptoms = [symptom.strip("[]' ") for symptom in user_symptoms]
        predicted_disease = get_disease_name(user_symptoms)
        desc, pre, med, die, wrkout = get_details(predicted_disease)

        my_pre = []
        for i in pre[0]:
            my_pre.append(i)

        return render_template("index.html", predicted_disease=predicted_disease, dis_desc=desc, des_pre=my_pre,
                               dis_med=med, dis_workout=wrkout,dis_die=die)
